...
cv.err$delta
# In this case, two numbers are identical (up to two decimal places) and correspond to the LOOCV.
# Out cross-validation estimate for the test error is approximately 24.23
# We can repeat this procedure for increasingly complex polynomial fits. 
# To automate this process, we use the for() loop function to initiate a for-loop,
# which iteratively fits polynomial regression for polynomials of order i=1 to i=5,
# and computes the associated cross-validation error, and stores it in "i"th element of,
# the vector cv.error.
# We begin by initializing the vector. 
# This might take couple of minutes to run on your laptop based on it's computing power! :) :)
cv.error = rep(0,5)
for(i in 1:5){
